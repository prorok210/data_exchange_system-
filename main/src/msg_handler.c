...
static void process_network_data_to_fc(uint8_t *data, int size) {
    ESP_LOGI(TAG, "Обработка данных из сети для полетного контроллера, размер: %d", size);
    
    mavlink_message_t msg;
    mavlink_status_t status;
    int processed = drone_msg_decode(data, size, &msg, &status);
    
    if (processed > 0) {
        ESP_LOGI(TAG, "Получено MAVLink сообщение:");
        drone_msg_log(&msg);
        
        // Отправка на UART 
        uint8_t buf[MAVLINK_MAX_PACKET_LEN];
        int len = drone_msg_encode(&msg, buf, sizeof(buf));
        if (len > 0) {
            uart_send_data(buf, len);
        }
    } else {
        ESP_LOGW(TAG, "Не удалось декодировать MAVLink сообщение");
    }
}

static void process_fc_data_to_network(uint8_t *data, int size) {
    ESP_LOGI(TAG, "Обработка данных от полетного контроллера для сети, размер: %d", size);
    ...
    if (processed > 0) {
        ESP_LOGI(TAG, "Получено MAVLink сообщение от полетного контроллера:");
        drone_msg_log(&msg);
        
        // Подготовка для отправки по сети
        uint8_t buf[MAVLINK_MAX_PACKET_LEN];
        int len = drone_msg_encode(&msg, buf, sizeof(buf));
        ...
        if (len > 0) {
            ...
            for (int i = 0; i < route_table_size; i++) {
                ...
                if (err) {
                    ESP_LOGE(TAG, "Ошибка отправки в сеть: 0x%x", err);
                    log_mac(route_table[i].addr);
                }
            }
        }
    } else {
        ESP_LOGW(TAG, "Не удалось декодировать MAVLink сообщение от контроллера");
    }
}
...
void esp_mesh_p2p_tx_main(void *arg)
{
    ...
    while (is_running) {
        // Проверяем наличие данных от UART 
